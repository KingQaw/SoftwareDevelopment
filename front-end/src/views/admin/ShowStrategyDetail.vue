...
const likeStrategy = async () => {
  if (strategyDetail.value.isLiked) {
    try {
      console.log(params);
...
const favoriteStrategy = async () => {
  ...
  if (strategyDetail.value.isFavorited) {
    try {
      console.log(params);
  ...
  <div class="comment-container">
    <div class="user-play"
      ...
      <div class="comment" style="width: 70%;">
        <el-input placeholder="良言一句三冬暖，恶语相向六月寒" v-model="myComment" style="width: 90%;" disabled></el-input>
        <el-button type="primary" plain @click="publishComment" disabled>{{ "发布评论  " + strategyDetail.commentCount }}</el-button>
      </div>
      <div class="likes-favorites">
        <el-button icon="check" color="brown" :plain="!strategyDetail.isLiked" @click="likeStrategy" disabled>{{ "点赞 " +
          strategyDetail.likeCount }}</el-button>
        <el-button icon="star" color="green" :plain="!strategyDetail.isFavorited" @click="favoriteStrategy" disabled>{{ "收藏 " +
